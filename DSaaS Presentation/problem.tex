...
% 	\tikzstyle{obj}=[draw, rectangle, minimum height=1cm, minimum width=2.2cm];
    ...
    \node(ver_B) [right = 1cm of ver_A]  {\file{B}{primary_dark}};
    \draw[->] (ver_A) -- (ver_B);

    \pause

    \node(ver_C) [right = 0cm of ver_B]  {\file{C}{primary_dark}};
    ...
    \node(ver_H) [right = 0cm of ver_G]  {\file{H}{primary_dark}};

    \pause

    \node(ver_last) [right = 1cm of ver_H] {\file{L}{primary_dark}};


    \node(copy-last) [right = 1cm of copy-c] {\file{Z}{secondary}};
    \draw[dashed,->,color=secondary](copy-c) -- (copy-last);

    \draw[dashed,->] (ver_H) -- (ver_last);
    \node(question) [right = 6cm of ver_B] {\huge ?};
